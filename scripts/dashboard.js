...
function filterForCity() {
    let filterForCityElement = document.getElementById('filter-for-city');
    let selectedCity = filterForCityElement?.value;

    let eventTrDisplay = selectedCity === 'All' ? 'table-row' : 'none';

    let eventTrElements = document.querySelectorAll('.event-tr');
    eventTrElements.forEach((eventTrElement) => {
        eventTrElement.style = `display: ${eventTrDisplay}`;
    });

    let cityElements = document.querySelectorAll(`.${selectedCity}`);
