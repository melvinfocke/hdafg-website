...
module.exports = async function () {
    ...
    return {
        ...
        steps: [
            ...
            {
                mainHeadline: 'Bearbeite Event - Ort',
                mainP: 'In wel&shy;chem Ort fin&shy;det das Event statt?',
                previewContent: function (setup) {
                    let object = combineStringObjectArray([setup.tempData, setup.data, setup.currentPageData]);
                    let event = eventArray.find((event) => event._id == object.eventId);
                ...
                inputs: [
                    {
                        ...
                        values: function (setup) {
                            ...
                            allCitiesArray?.forEach((city) => {
                                if (city !== event.city) array.push(city);
                            });
                            return array;
                        }
                    }
                ...
                validate: function (userInput) {
                    if (userInput.id !== 'city') return { message: "userInput.id muss 'city' entsprechen." };
                    let message = '';
                    let exists = false;
                    const allCitiesStrArray = ALL_CITIES.split('{|,|}');
                    allCitiesStrArray.forEach((cityStr) => {
                        if (cityStr.split('{|:|}')[0] == userInput.value) exists = true;
                    });

                    if (exists == false) {
                        message = `city '${userInput.value}' existiert nicht.`;
                    }
                    return message ? { message } : null;
                }
            },
